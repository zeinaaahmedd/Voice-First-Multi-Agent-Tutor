...
# ── Gemini (LLM — cloud, no local GPU needed) ───────────────────────────────
...
class PedagogicalAgentGraph:
    ...
    def call_model(self, state: AgentState) -> dict:
        """Processes the state, injects RAG context, and hits Gemini API."""
        ...
        # Gemini uses roles "user" and "model" (not "assistant"), and the
        # system prompt goes into a separate "system_instruction" field.
        gemini_contents = []
        ...
        for msg in state["messages"]:
            if isinstance(msg, HumanMessage):
                ...
                if msg == last_user_message:
                    text = f"{context_wrapper}{msg.content}"
                else:
                    text = msg.content
        ...
        payload = {
            ...
            "generationConfig": {
                "temperature": 0.7,
                # Enough room for a full pedagogical explanation including RAG context.
                "maxOutputTokens": 4096,
                # Cap thinking tokens so they don't eat into the output budget.
                # 0 disables thinking entirely — fine for conversational teaching.
                "thinkingConfig": {
                    "thinkingBudget": 0
                },
            },
        ...
        try:
            print(f"LOG: Calling Gemini ({GEMINI_MODEL}) with conversation history...")
            ...
            # Log finish reason so truncation is visible in the console.
            finish_reason = candidate.get("finishReason", "UNKNOWN")
            if finish_reason not in ("STOP", ""):
                print(f"LOG: Gemini finishReason={finish_reason} — response may be incomplete.")

            # Gemini 2.5 Flash with thinking returns multiple parts; some have
            # "thought": true (internal reasoning). Collect only the real output.
            parts = candidate.get("content", {}).get("parts", [])
            text_parts = [p["text"] for p in parts if not p.get("thought", False) and "text" in p]
            ai_reply = "".join(text_parts).strip()

            if not ai_reply:
                ai_reply = "عذراً، الرد كان فاضي. حاول تاني."

        except requests.exceptions.Timeout:
            ai_reply = "خطأ: Gemini ما ردتش في الوقت المطلوب. جرب تاني."
        except requests.exceptions.ConnectionError:
            ai_reply = "خطأ: مش قادر اتصل بـ Gemini. تاكد من الانترنت."
...
if __name__ == "__main__":
    ...
    print(f"  LLM : {GEMINI_MODEL} (Gemini API)")
